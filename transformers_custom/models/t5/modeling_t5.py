...
class T5ForConditionalGenerationWithLatentSpace(T5PreTrainedModel):
    ...
    def __init__(self, config, 
                ...
                mask_non_target_z_vector=True,
                separate_targetattr_head=False,
                do_mi=False,
                z_tar_vector_dim=1,
                latent_space_type='plain', # ['plain', 'vae', 'wae', 'adversarial']
                # use_latent_enc_dec_for_plainae=False,
                wae_z_enc_type=None, # ['deterministic', 'stochastic', None] None is same as stochastic
                separate_latent_enc=None,
                separate_latent_dec=None,
                ...
                latent_size=None, # if None, will use hidden state as latent vector (z)
                ):
        super().__init__(config)
        ...
        # self.use_latent_enc_dec_for_plainae = use_latent_enc_dec_for_plainae
        ...
        if self.latent_space_type in ['vae', 'wae']:
            ...
            if z_tar_vector_dim > latent_size:
                raise ValueError("z_tar_vector_dim of size {} cannot be larger than latent_size of size {} in vae mode".format(z_tar_vector_dim, latent_size))
        ...
        # elif use_latent_enc_dec_for_plainae:
        #     self.vae_enc = nn.Linear(self.model_dim, self.latent_size, bias=False)
        #     self.vae_dec = nn.Linear(self.latent_size, self.model_dim, bias=False)
    ...
    def parallelize(self, device_map=None):
        ...
        # if self.latent_space_type in ['vae', 'wae'] or use_latent_enc_dec_for_plainae:
        #     self.vae_enc = self.vae_enc.to(self.decoder.first_device)
        #     self.vae_dec = self.vae_dec.to(self.decoder.first_device)
    ...
    def forward(
        ...
        # Encode if needed (training, first prediction pass)
        if encoder_outputs is None:
            # print("forward, encoder_outputs is None")
        ...
        hidden_states = encoder_outputs[0]
        # print("hidden_states.shape: ", hidden_states.shape) # torch.Size([16, 85, 512])

        # print("forward, hidden_states.shape: ", hidden_states.shape)
    
        # Add latent space code here before decoder step: start

        # print("before latent, hidden_states.shape: ", hidden_states.shape) # [batch, length, dim] torch.Size([16, 85, 1024])
        ...
        if self.latent_space_type in ['vae', 'wae']:
        # if self.latent_space_type in ['vae', 'wae']:
            # if self.model_parallel and self.separate_latent_enc == True:
            #     self.vae_enc = self.vae_enc.to(self.encoder.first_device)
                # torch.cuda.set_device(self.encoder.first_device)
                # self.vae_enc = self.vae_enc.to(self.encoder.first_device)
                # self.vae_dec = self.vae_dec.to(self.encoder.first_device)

            vae_enc_input = pooled_hidden_states.squeeze(dim=1) # remove len dim
            # print("pooled_hidden_states.shape: ", pooled_hidden_states.shape)
            if self.wae_z_enc_type == 'deterministic':
                if self.separate_latent_enc == True:
                    ...
                    # print("A1 latent_vector.shape: ", latent_vector.shape)
            else:
                # print("Check2")
                if self.model_parallel:
            ...
            # if use_latent_enc_dec_for_plainae:
            #     if self.model_parallel:
            #         torch.cuda.set_device(self.encoder.first_device)
            #         self.vae_enc = self.vae_enc.to(self.encoder.first_device)
            #         self.vae_dec = self.vae_dec.to(self.encoder.first_device)
            ...
            #     latent_vector = pooled_hidden_states
            latent_vector = pooled_hidden_states
            # print("A2 latent_vector.shape: ", latent_vector.shape)

        # print("pooled_hidden_states.shape: ", pooled_hidden_states.shape) # torch.Size([16, 1, 1024])
        z_tar = latent_vector[:, :, :self.z_tar_vector_dim] # take first dim as predicted contrast value, torch.Size([16, 1, z_tar_vector_dim])
        # z_tar = latent_vector[:, :, :self.z_tar_vector_dim].clone() # take first dim as predicted contrast value, torch.Size([16, 1, z_tar_vector_dim])
        # print("z_tar.shape: ", z_tar.shape) #  torch.Size([16, 1, 1]) 
        if self.do_mi:
            z_nontar = latent_vector[:, :, self.z_tar_vector_dim:]
            # print("z_nontar.shape: ", z_nontar.shape)
            # Set device for model parallelism
            if self.model_parallel:
                # torch.cuda.set_device(self.encoder.first_device)
            ...
            if train_mi_head_step:
                ...
                # print("z_nontar.detach()")

            nontar_value_pred = self.mi_head(z_nontar) # torch.Size([16, 1, 1])
            # print("a nontar_value_pred.shape: ", nontar_value_pred.shape)
            nontar_value_pred = torch.squeeze(nontar_value_pred, -1) # torch.Size([16, 1])
            # print("b nontar_value_pred.shape: ", nontar_value_pred.shape)
        ...
        if self.separate_targetattr_head:
            ...
            if self.model_parallel:
                ...
                # torch.cuda.set_device(self.encoder.first_device)
                # self.targetattr_head = self.targetattr_head.to(self.encoder.first_device)

            # print("z_tar.device: ", z_tar.device)
            # print("self.targetattr_head.weight.device: ", self.targetattr_head.weight.device)
            value_pred = self.targetattr_head(z_tar) # torch.Size([16, 1, 1])
            # print("A value_pred.shape: ", value_pred.shape)
            # print("self.targetattr_head.weight: ", self.targetattr_head.weight)
            # print("self.targetattr_head.weight.shape: ", self.targetattr_head.weight.shape) # torch.Size([1, 256])
            # print("self.targetattr_head.weight normalized: ", F.normalize(self.targetattr_head.weight) ) # torch.Size([1, 256])
            # print("self.targetattr_head.weight normalized shape: ", F.normalize(self.targetattr_head.weight).shape ) # torch.Size([1, 256])
        else:
            value_pred = z_tar
            # print("B value_pred.shape: ", value_pred.shape)
        ...
        if contrast_targets is not None:
            ...
            # print("B contrast_labels: ", contrast_labels) #  tensor([[0.5000, 0.0000, 1.0000, 1.0000, 1.0000, 0.0000, 0.0000, 0.0000, 1.0000,.. , 0.5 at the diagonal
            ...
            # print("loss_mask: ", loss_mask)
            # print("B losses: ", losses)  # torch.Size([16, 16])
            ...
            if return_same_label_loss:
                ...
                # print("value_pred_diff: ", value_pred_diff)
                # print("torch.sum(value_pred_diff*same_label_loss_mask): ", torch.sum(value_pred_diff*same_label_loss_mask))
                # print("torch.sum(same_label_loss_mask): ", torch.sum(same_label_loss_mask))
                same_label_loss = torch.sum( torch.abs(value_pred_diff*same_label_loss_mask) ) / torch.sum(same_label_loss_mask)
                # print("A same_label_loss: ", same_label_loss)

            # TODO: diff label loss
        ...
        if self.mask_non_target_z_vector:
            ...
            # print("mask_non_target_z_vector, latent_vector: ", latent_vector)

        # add z_tar_edit_before_dec to latent_vector
        # print("if z_tar_edit_before_dec is not None:, latent_vector[:, :, :self.z_tar_vector_dim].shape: ", latent_vector[:, :, :self.z_tar_vector_dim].shape)
        # print("if z_tar_edit_before_dec is not None:, z_tar.shape: ", z_tar.shape)
        if z_tar_edit_before_dec is not None:
            ...
            else: # z edit is a vector
                # vector edit to z
                z_tar_attr_direction = F.normalize(self.targetattr_head.weight) # torch.Size([1, self.z_tar_vector_dim])
                z_tar_attr_direction = z_tar_attr_direction / torch.norm(self.targetattr_head.weight, dim=-1) # so that value_pred shifts by z_tar_edit_before_dec magnitude
                z_tar_attr_direction = z_tar_attr_direction.unsqueeze(0) # torch.Size([1, 1, self.z_tar_vector_dim])
                z_tar_edit_tensor = z_tar_edit_before_dec * z_tar_attr_direction
                latent_vector[:, :, :self.z_tar_vector_dim] = z_tar + z_tar_edit_tensor

                # # debug added perturbation
                # value_pred_unperturbed = self.targetattr_head(z_tar)
                # z_tar_perturbed = latent_vector[:, :, :self.z_tar_vector_dim]
                # value_pred_perturbed = self.targetattr_head(z_tar_perturbed)
                # print("value_pred_perturbed - value_pred_unperturbed: ", value_pred_perturbed - value_pred_unperturbed)

        if self.latent_space_type in ['vae', 'wae'] and self.separate_latent_dec != False:
            # vae latent decoding to T5decoder hidden state
            # print("Check3")
            if self.model_parallel:
                self.vae_dec = self.vae_dec.to(latent_vector.device)
            pooled_hidden_states = self.vae_dec(latent_vector)
        else:
            pooled_hidden_states = latent_vector
            # print("C pooled_hidden_states.shape: ", pooled_hidden_states.shape)

        # print("hidden_states.shape: ", hidden_states.shape) # torch.Size([64, 85, 1024])          
        # replace hidden_states with pooled_hidden_states
        if self.pool_enc_hidden_states_for_dec:
            hidden_states = pooled_hidden_states
        # print("pooled_hidden_states.shape: ", pooled_hidden_states.shape) # torch.Size([64, 1, 1024])
        
        # print("after latent, hidden_states.shape: ", hidden_states.shape) # [batch, length, dim] torch.Size([16, 85, 1024])
        ...
        #     print("decoder_outputs = self.decoder(, decoder_input_ids: ", decoder_input_ids)
        # else:
        #     print("decoder_outputs = self.decoder(, decoder_input_ids.shape: ", decoder_input_ids.shape)
        # if decoder_inputs_embeds is None:
        #     print("decoder_outputs = self.decoder(, decoder_inputs_embeds: ", decoder_inputs_embeds)
        # else:
        #     print("decoder_outputs = self.decoder(, decoder_inputs_embeds.shape: ", decoder_inputs_embeds.shape)
        # print("before self.decoder, hidden_states.shape: ", hidden_states.shape)

        # Decode
        # print("decoder_outputs = self.decoder(, hidden_states.shape: ", hidden_states.shape)
        # if attention_mask is not None:
            # print("decoder_outputs = self.decoder(, attention_mask.shape: ", attention_mask.shape)
        # if past_key_values is not None:
            # print("decoder_outputs = self.decoder(, past_key_values: ", past_key_values)

        # print("B hidden_states: ", hidden_states)
        # print("B decoder_input_ids: ", decoder_input_ids)
        # print("B decoder forward")
        # print("B decoder_input_ids: ", decoder_input_ids)
        # print("B decoder_attention_mask: ", decoder_attention_mask)
        # print("B decoder_inputs_embeds: ", decoder_inputs_embeds)
        # print("B past_key_values: ", past_key_values)
        # if past_key_values is not None:
        #     print("B past_key_values.shape: ", past_key_values.shape)
        # print("B hidden_states: ", hidden_states)
        # print("B attention_mask: ", attention_mask)
        # print("B decoder_head_mask: ", decoder_head_mask)
        # print("B head_mask: ", head_mask)
        # print("B use_cache: ", use_cache)
        # print("B output_attentions: ", output_attentions)
        # print("B output_hidden_states: ", output_hidden_states)
        # print("B return_dict: ", return_dict)
        # self.decoder.eval()
        decoder_outputs = self.decoder(
            input_ids=decoder_input_ids,
            attention_mask=decoder_attention_mask,
        ...
        # print("B sequence_output: ", sequence_output)
        # print("B sequence_output.shape: ", sequence_output.shape)
        ...
        if labels is not None:
            ...
            # print("B labels: ", labels) # tensor([[ 2, 86, 87,  ..., 79, 84,  1],
            # print("B labels.shape: ", labels.shape) # torch.Size([16, 85])
            # print("B lm_logits.shape: ", lm_logits.shape) # torch.Size([16, 85])
